...
let cartele = document.getElementById("cart-page");
cartele.style.display = "none";
let foodele = document.getElementById("food");
foodele.style.display = "block";

/**
 * This function take input from user on category and called displayItem
 */
...
function displayItem(item) {
  let filteredItems = foodItem.filter((s) => s.category === item);
...
function displayItems(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    document.getElementById(
      "items"
    ).innerHTML += `<div class="row text-center m-b-5">
          <div class="col-md-3 text-center">
            <p class="fw-light text-break"><img src="${items[i].img}" width="150px" height="150px"></p></h3>
          </div>
          <div class="col-md-2">
            <p class="fw-light text-break">${items[i].name}</p>
          </div>
          <div class="col-md-2">
            <p class="fw-light text-break">${items[i].price}</p>
          </div>
          <div class="col-md-2">
            <p class="fw-light text-break">${items[i].rating}</p>
          </div>
          <div class="col-md-2">
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="chk" id='ch${items[i].id}' onClick='addToCart(${items[i].id})'>
          </div>
          </div>
          <hr>
          <div>`;
...
function addToCart(itemid) {
  ...
  let itemToAdd = foodItem.find((item) => item.id === id);
  let ele = document.getElementById(`ch${itemid}`);

  //if(ele.checked===false) alert('Please select at least one item')
  console.log("itemToAdd" + itemToAdd);
  if (cartData.indexOf(itemToAdd) == -1 && ele.checked) {
    cartData.push(itemToAdd);
    ele.checked = true;
  } else if (cartData.indexOf(itemToAdd) !== -1 && ele.checked === false) {
    ...
    alert("Items alread added in the card");
    ele.checked = true;
  }
...
/**
 * Display all the items that user has selected
 */
function cartItems() {
  ...
  for (let i = 0; i < cartData.length; i++) {
    ...
    <td>
          <button   id="${cartData[i].id}" onclick="decQuantity(event)">-</button>
          ${cartData[i].quantity}
          <button id="${cartData[i].id}" onclick="incQuantity(event)">+</button>
    </td>
    <td>${cartData[i].price}</td>
    <td>
    <button  id="${cartData[i].id}" onclick="removeItems(event)">X</button> 
    </td>
    </tr>`;
  }
  totalAmount();
...
/**
 *
 * @param event takes an event from onclick method incQuantity
 */
function incQuantity(event) {
  let id = parseInt(event.target.id);
  console.log(id);
  let filterInc = cartData.find((item) => {
    return item.id === id;
  });
  filterInc.quantity += 1;
  //(oldprice*qty - (newprice*(qty-1)))/(totalqty-1)
  let latestPrice =
    (filterInc.price * filterInc.quantity -
      filterInc.price * (filterInc.quantity - 1)) /
    (filterInc.quantity - 1);
  filterInc.price = latestPrice * filterInc.quantity;
...
/**
 *
 * @param  event takes an event from onclick method decQuantity
 */
function decQuantity(event) {
  let id = parseInt(event.target.id);
  console.log(id);

  let ele = document.getElementById(`ch${id}`);
  let filterdec = cartData.find((item) => {
    return item.id === id;
  });

  let idx = cartData.indexOf(filterdec);
  console.log("item check " + idx);
  if (filterdec.quantity > 1) {
    //(oldprice*qty - (newprice*(qty-1)))/(totalqty) -- we are calulating then dec
    let latestPrice =
      (filterdec.price * filterdec.quantity -
        filterdec.price * (filterdec.quantity - 1)) /
      filterdec.quantity;
    filterdec.quantity -= 1;
    filterdec.price = filterdec.quantity * latestPrice;
  } else {
    cartData.splice(idx, 1);
    document.getElementById("cart-plus").innerText =
      " " + cartData.length + " Items";
...
/**
 *
 * @param  event remove the particular items from the cart
 */
function removeItems(event) {
  let id = parseInt(event.target.id);
  console.log(id);
  let remove = cartData.find((item) => {
    return item.id === id;
  });
  let idx = cartData.indexOf(remove);
  cartData.splice(idx, 1);
